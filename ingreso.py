...
def mostrar_formulario():
    ...
    mat1 = st.number_input("MAT 1 (Hz)", value=60.0)
    vel_motor_a = st.number_input("Velocidad Motor A", value=65.0)
    z1 = st.number_input("Temperatura Z-1 (°C)", value=195.0)
    otc_01 = st.number_input("OTC-01 (°C)", value=90.0)
    aceptada = st.selectbox("¿Corrida Aceptada?", ["SI", "NO"]) == "SI"

    if st.button("Guardar corrida"):
        conn = get_connection()
        cursor = conn.cursor()
        cursor.execute("""
